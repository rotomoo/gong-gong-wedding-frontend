...
function ConsultationChatPage() {
  const { requestId, proposalId } = useParams();
  const navigate = useNavigate();
  const { markNotificationAsRead, myRequests } = useNotifications();

  const request = myRequests.find(req => req.id === parseInt(requestId));
  const proposal = request?.proposals?.find(p => p.id === parseInt(proposalId));

  const [messages, setMessages] = useState([]);

  useEffect(() => {
    if (proposal && request && proposal.newMessagesCount > 0) {
  ...
  }, [proposal, request, markNotificationAsRead]);
