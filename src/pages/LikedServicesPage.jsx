...
import { ShieldCheckIcon, StarIcon, HeartIcon, XMarkIcon, ChevronDownIcon } from '@heroicons/react/24/solid';
...
function LikedServicesPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      setLikedServiceIds(JSON.parse(saved));
    }
  }, []);
  ...
  const filteredAndSortedLikedServices = useMemo(() => {
    let result = weddingHalls.filter(service => likedServiceIds[service.id]);

    result = result.filter(hall => hall.name.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    return result;
  }, [likedServiceIds, searchTerm, filters, sortBy]);
    ...
    <div className="bg-white min-h-screen">
      <div className="container mx-auto px-4 py-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900">찜한 서비스</h1>
          <p className="mt-4 text-lg text-gray-500 max-w-2xl mx-auto">마음에 드는 서비스를 모아보고, 비교해보세요.</p>
        </div>
        ...
        {filteredAndSortedLikedServices.length > 0 ? (
          ...
          <div className="text-center py-20">
            <p className="text-xl text-gray-500">찜한 서비스가 없습니다.</p>
            <p className="mt-2 text-gray-400">서비스 둘러보기에서 마음에 드는 서비스의 하트를 눌러보세요.</p>
...
export default LikedServicesPage;
