...
function MyPage() {
  ...
  // '상담 중' 상태를 제외한 실제 예약 건수만 계산
  const bookingHistoryCount = bookings.filter(b => b.status !== '상담 중').length;

  const mainActions = [
    { title: '내 예약 내역', description: '예약 및 서비스 내역을 확인합니다.', count: bookingHistoryCount, icon: CalendarDaysIcon, link: '/bookings', newCount: 0 },
